...
Az eredeti algoritmust a BCPL-ről írt könyvben definiálták BCPL-ben és szöveges formában \cite{richards_basic_1984}, de egy verziója elérhető példakóddal az ANTLR3 eszközhöz is \cite{sam_harwell_operator_2008}.
...
\begin{figure}[htb]
    \includegraphics[width=\linewidth]{figures/ffm.pdf}
...
Az utolsó lépésben kerül előállításra az előzőekben már említett LLVM \gls{IR}.
Ez egy olyan formátuma az LLVM rendszernek, amelyiket bemenetként várnak a különböző alrendszerek.
Ez szolgál egy olyan rétegként, amelyik elég általános ahhoz, hogy ne kötődjön egy konkrét hardverhez, de elég alacsony szintű ahhoz, hogy a lehető legtöbb olyan koncepciót, amivel egy-egy platform assembly nyelve rendelkezik képes kifejezni.
...
Az ennek a feltételnek megfelelő utasítások sorát pedig atomi módon lefutó egyszerű blokkokba (basic block) kell rendezni, amelyek között ugrásokkal lehet váltani a blokk végén.
Ez lehet feltételes ugrás.
...
\begin{figure}[htb]
    ...
    \begin{subfigure}{.7\linewidth}
        \includegraphics[width=\linewidth]{figures/ssa.pdf}
        \caption{\gls{SSA} átírás}
    \end{subfigure}
    \hfil
    \caption{Pszeudokód példa átírása \gls{SSA} formába}
\end{figure}

Ennek a sémának az előnye az optimalizáló megvalósításában jelenik meg, amelyiket -- és a hozzá fejlesztett egyes optimalizációs lépéseket -- így jelentősen egyszerűbb megvalósítani, mert ...
\todo[inline]{Ide meg kéne magyaráznom, hogy miért, de igazából nem tudok értelmes érvet rá azon kívül, hogy hát látszik.}

A kódgenerálás szempontjából ennek két következménye van.
Első, hogy ha egy változó többször kap értéket egy program folyama során, akkor azt olyan módon kell \gls{SSA} formába hozni, hogy minden értékadásnál egy új verziót kell hozzá bevezetni.
Az egyszerű blokkok szerint jelenleg megfelelő verziót mindig számon kell tartani, és a változóra vonatkozó hivatkozásokat a erre a példányra kell feloldani.
Ez a C4 megvalósításban nem játszik szerepet, hiszen minden immutábilis a gazdanyelvben is, így nem kell ezzel külön foglalkozni, automatikusan megfelel az elvárásoknak.

A másik, hogy az elágazó \gls{CFG} esetén a $\varphi$ függvény megfelelő paraméterezését le kell generálni a blokkok elejére, ahol több beérkező él is van.
A C4 megvalósításakor ezzel sem kell foglalkozni, mert nincs külön elágazás vagy ciklus eleme a nyelvnek, ami nehézségeket okzna az {\tt if/3} például is csak egy olyan függvény, mint \aref{fig:c4-ffm}.~ábrán definiált {\tt add/2}.
Meghívása pedig emiatt teljesen azonos, és így nincs elágazás.

Kérdés lehet, hogy ha egyáltalán nincs elágazás, akkor hogyan lehetséges megvalósítani azt, hogy a nyelvben mégis elérhető és használható az {\tt if/3} függvény.
...
Ez a megadása az értékeknek megfelel a $\lambda$-kalkulusban definiált Church-boolean \cite{kemp_theoretical_2007} koncepciónak, amivel a logikai értékeket és olyan logikai függvényeket, mint az ÉS és a VAGY lehet megvalósítani $\lambda$-kalkulusban.
...
\begin{figure}[htb]
    ...
    \begin{subfigure}{.45\linewidth}
        \begin{lstlisting}[gobble=12,numbers=left]
            # A beépített függvényekkel való névütközést
            # megelőzendő
            let ctrue { { |t f| &(t)/0 } }
            let cfalse { { |t f| &(f)/0 } }
...
Erről x\todo{link}
részben leírtak alapján van könnyedén lehetőség.
