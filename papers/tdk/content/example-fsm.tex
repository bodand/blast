...
A sokak által ismert és egyszerű véges állapotgépek (Finite State Machine (FSM))  leírására adott szakterület-specifikus nyelv jó példa tud lenni egy DSL készítésére képes eszköz számára.
...
Ez kifejezetten egy előnye egy DSL-nek, míg egy általános célú nyelvben ezt gyakran jóval bonyolultabban lehet megvalósítani.
...
függelékben, egy példaként megvalósított állapotgép látható \aref{fig:ex-stm}.~ábrán. A DSL-ben írt kód a pedig alább következik \aref{lst:ex-fsm}.~listázásban.
...
\begin{lstlisting}[gobble=2,numbers=left,caption={C4 állapotgép DSL használati példa},label=lst:ex-fsm]
	let myfsm fsm
		start state "start"
...
Láthatólag ez egy rövid kódrészlet, az alapvetően egyszerű trükkök mind el vannak rejtve a DSL megvalósítása mögé, így az állapotgépet használónak csak azzal kell foglalkoznia, amivel szeretne.
